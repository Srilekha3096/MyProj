...
const ViewMaterialReceiptConversion = ({
    scrollHeight,
    setListDesignation,
    setViewDesignation,
    currentDesignations,
        ...
        if (Id) {
            axios
                ...
                .then((res) => {
                    ...
                    axios
                        .get(
                            `${BASE_URL}/Erpapp/Purchaserequestworkflow/?Request_Id=${res.data?.MatReq_Id}`
                        )
                        .then((res) => {
            ...
            axios
                .get(
                    `${BASE_URL}/Erpapp/MaterialRequestCRUD/?id=${currentDesignations?.id}`,
                    header
                )
                .then((res) => {
                    console.error(res?.data);
            ...
            axios
                .get(
                    `${BASE_URL}/Erpapp/Purchaserequestworkflow/?Request_Id=${currentDesignations?.MatReq_Id}`
                )
                .then((res) => {
                    console.log("REE", res?.data);
                    setApprovalStatus(res?.data[0]?.Approve);
            ...
            axios
                .get(
                    `${BASE_URL}/Erpapp/Updaterecordsfilter/?Partner_Id=${Partner_Id}&Document_Id=${currentDesignations?.MatReq_Id}`,
                    header
                )
                .then((res) => {
                    console.error("data", res?.data);
            ...
            <Div className="row">
                <Div className="col-sm-12 col-md-10">
                    <Typography variant="h3" sx={{ fontWeight: 600 }}>
                        Material Conversion Receipt Overview
                    </Typography>
                </Div>
                ...
                <Div>
                    <TabContext value={value}>
                        ...
                        <TabPanel value="1" sx={{ p: 1.5, minHeight: "350px" }}>
                            <Div className="row mt-1">
                                <Div className="col-5 col-lg-4 col-xl-2">
                                    <Typography variant="h4">MC Receipt No</Typography>
                                    <Typography variant="h4">MC Issue No</Typography>
                                    <Typography variant="h4">Business Unit</Typography>
                                    <Typography variant="h4">Received By</Typography>
                                    <Typography variant="h4">Receipt Date</Typography>
                                    <Typography variant="h4">Store Name</Typography>
                                </Div>
                                <Div className="col col-lg-5 col-xl-4">
                                    <Typography variant="h4">
                                        : {currentDesignations?.Matcon_Receipt_Id}
                                    </Typography>
                                    <Typography variant="h4">
                                        : {currentDesignations?.Material_Issue_No}
                                    </Typography>
                                    <Typography variant="h4">
                                        :{" "}
                                    ...
                                    <Typography variant="h4">
                                        :{" "}
                                        {currentDesignations?.Received_By &&
                                            currentDesignations?.Received_By}
                                    </Typography>
                                    <Typography variant="h4">
                                        :{" "}
                                        {currentDesignations?.Receipt_Date && <DateFormatter date={currentDesignations?.Receipt_Date} />}
                                    </Typography>
                                    <Typography variant="h4">
                                        :{" "}
                        ...
                        <TabPanel value="3" sx={{ p: 1.5, minHeight: "350px" }}>
                            <HistoryOverview updatedHistory={updateHistory} itemId={"MC Receipt No"} />
                        </TabPanel>
                    </TabContext>
                </Div>
            </Div>
...
export default ViewMaterialReceiptConversion
